...
via apparmor_parser(8), typically through the F</etc/init.d/pyronia>
SysV initscript, which is used like this:

	# /etc/init.d/pyronia start
	# /etc/init.d/pyronia stop
	# /etc/init.d/pyronia restart
...
Profiles are traditionally stored in files in F</etc/pyronia.d/>
...
filesystem:
	...
	$ cat /sys/kernel/security/pyronia/profiles
	/usr/bin/mutt
	/usr/bin/gpg
	   ...
...
to access, the kernel will report a message through audit, similar to:
	...
	audit(1386511672.612:238): pyronia="DENIED" operation="exec" 
	  parent=7589 profile="/tmp/sh" name="/bin/uname" pid=7605 
	  comm="sh" requested_mask="x" denied_mask="x" fsuid=0 ouid=0

	audit(1386511672.613:239): pyronia="DENIED" operation="open" 
	  parent=7589 profile="/tmp/sh" name="/bin/uname" pid=7605 
	  comm="sh" requested_mask="r" denied_mask="r" fsuid=0 ouid=0

	audit(1386511772.804:246): pyronia="DENIED" operation="capable"
...
reported to audit will be of the form:
	...
	audit(1386512577.017:275): pyronia="ALLOWED" operation="open"
	  parent=8012 profile="/usr/bin/du" name="/etc/pyronia.d/tunables/"
	  pid=8049 comm="du" requested_mask="r" denied_mask="r" fsuid=1000 ouid=0

	audit(1386512577.017:276): pyronia="ALLOWED" operation="open"
	  parent=8012 profile="/usr/bin/du" name="/etc/pyronia.d/tunables/"
	  pid=8049 comm="du" requested_mask="r" denied_mask="r" fsuid=1000 ouid=0
...
=item F</etc/init.d/pyronia>

=item F</etc/pyronia.d/>

=item F</var/lib/pyronia/>
...
pyronia_parser(8), aa_change_hat(2), pyronia.d(5),
