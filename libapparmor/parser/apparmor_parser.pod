...
"/sys/kernel/security/pyronia").

=item -M n, --features-file n

Use the features file located at path "n" (default is
/etc/pyronia.d/cache/.features). If the --cache-loc option is present, the
...
defaults to /etc/pyronia.d/cache
...
An optional config file /etc/pyronia/parser.conf can be used to specify the
