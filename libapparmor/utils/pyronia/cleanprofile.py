...
class Prof(object):
    def __init__(self, filename):
        self.aa = pyronia.aa
        self.filelist = pyronia.filelist
        self.include = pyronia.include
        ...
        for hat in sorted(self.profile.aa[program].keys()):
            ...
            for inc in includes:
                if not self.profile.include.get(inc, {}).get(inc, False):
                    pyronia.load_include(inc)
                deleted += pyronia.delete_duplicates(self.other.aa[program][hat], inc)

            #Clean duplicate rules in other profile
            for ruletype in pyronia.ruletypes:
                if not self.same_file:
