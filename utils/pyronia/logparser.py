...
import time
import LibAppArmor
from pyronia.common import AppArmorException, AppArmorBug, open_file_read, DebugLogger

from pyronia.aamode import validate_log_mode, log_str_to_mode, hide_log_mode, AA_MAY_EXEC
...
class ReadLog:
    ...
    RE_log_parts = [
        ...
        'type=(AVC|PYRONIA[_A-Z]*|' + RE_type_num + ')\s+' + RE_audit_time_id + '(type=' + RE_type_num + '\s+)?' + RE_aa_or_op,  # v2_6 audit and dmesg
        'type=USER_AVC\s+' + RE_audit_time_id + '.*pyronia=',  # dbus
        'type=UNKNOWN\[' + RE_type_num + '\]\s+' + RE_audit_time_id + RE_aa_or_op,
        'dbus\[[0-9]+\]:\s+pyronia=',  # dbus
    ]
